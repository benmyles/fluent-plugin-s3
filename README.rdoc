...
%{bsonid} adds a unique bson id to the filename.
...
[out_format] Optional output format, 'json' or 'csv'.

[csv_col_sep] Delimiter to separate CSV columns. Default is '|'.

[csv_sort_cols] Sorts the CSV columns based on the input hash keys. Defaut is true.
